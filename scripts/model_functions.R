...
get.partition <- function(label, descrs.all, p = 0.7){
...
svmModel <- function(train.set, test.set, probability=FALSE, weights=FALSE){
  ...
  tryCatch({
    ...
    # useful when data frame is only composed of only one feature and the label
    if(ncol(test.set) == 2){
      test.set.temp <- data.frame(test.set[,-which(colnames(test.set) == "LABEL")])
      colnames(test.set.temp) <- setdiff(colnames(test.set), "LABEL")
    } else { # all other cases
      test.set.temp <- test.set[, -which(colnames(test.set) == "LABEL")]
    }
